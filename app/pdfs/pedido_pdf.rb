class PedidoPdf < Prawn::Document
  ...
  def footer
    draw_text @pedido.user.nombre + " " + @pedido.user.apellidos, :at => [10, 28]
    draw_text "_________________________________", :at => [10, 25]
    draw_text "Elaboró y Autorizó", :at => [32, 12]

    draw_text @pedido.departamento.titular, :at => [310, 28]
    draw_text "_________________________________", :at => [310, 25]
    draw_text "Firma del Responsable", :at => [360, 12]
  end
  ...
  def articulos_rows
    [ [{:content => "Titular", :colspan =>2}, {:content => "Número de Orden", :colspan => 2}],
      ...
      [detalles_pedido.articulo_id, detalles_pedido.articulo.descripcion, detalles_pedido.solicitado.to_s.reverse.gsub(/(\d{3})(?=\d)/, '\\1,').reverse, detalles_pedido.surtido.to_s.reverse.gsub(/(\d{3})(?=\d)/, '\\1,').reverse]
    end
  end
end
